...
function calculate() {
    ...
    for (const epsilon of epsilonRange1) {
        for (const s of sRange) {
            const result = numeric.uncmin(objective, [epsilon, s], { tol: 1e-6, maxiter: 50 });
    ...
    // Sample height measurements
    const heights = [1.0, 1.2, 0.9, 1.1, 1.3, 1.0, 0.8, 1.0, 1.1, 1.2, 1.0];
    const meanHeight = math.mean(heights);
    const squaredDeviations = heights.map(height => Math.pow(height - meanHeight, 2));
    const rmsHeight = Math.sqrt(math.mean(squaredDeviations));

    // Topp equation to convert ε to soil moisture
    function toppEquation(epsilon) {
        return 4.3e-6 * Math.pow(epsilon, 3) - 5.5e-4 * Math.pow(epsilon, 2) + 2.92e-2 * epsilon - 5.3e-2;
    }
    ...
    function calculateMetrics(observed, predicted) {
        const mae = math.mean(observed.map((obs, i) => Math.abs(obs - predicted[i])));
        const mse = math.mean(observed.map((obs, i) => Math.pow(obs - predicted[i], 2)));
        const rmse = Math.sqrt(mse);
        const r2 = observed.length > 1 ? 1 - mse / math.mean(observed.map(obs => Math.pow(obs - math.mean(observed), 2))) : 'N/A';
        return { mae, mse, rmse, r2 };
    }
    ...
    const resultsDiv = document.getElementById("results");
    resultsDiv.innerHTML = ` 
        <p>Optimized Dielectric Constant (epsilon):<b> ${epsilonOpt}</b></p>
        <p>Estimated Soil Moisture:<b> ${soilMoisture}</b> </p>
        <p>Estimated Soil Roughness:<b> ${soilRoughness} meters</b> </p>
        <p>Modeled sigma_vv: <b>${modelSigmaVV}</b></p>
        <p>Modeled sigma_vh: <b>${modelSigmaVH.toExponential(2)}</b></p>`;

        plotData([sigmaVV_dB, sigmaVH_dB], [modelSigmaVV, modelSigmaVH]);
